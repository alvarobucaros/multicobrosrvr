...
controller.listQuery = (req, res) => {  
 ...
 controller.insertUser = (req, res) => {
   ...
   req.getConnection((err, conn) => {
    ...
    sql +=" us_localidad, us_tipoDoc, us_nroDoc, us_telefono, us_clave, us_estado, us_barrio )";
    sql += ' VALUES ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?, ? ) '
    console.log(sql);
      conn.query(sql, [ req.body.us_idEmpresa, req.body.us_nombre, req.body.us_email, req.body.us_direccion, req.body.us_ciudad, req.body.us_localidad, req.body.us_tipoDoc, req.body.us_nroDoc, req.body.us_telefono, md5(req.body.us_clave), req.body.us_estado, req.body.us_barrio], 
        (err, rows) => {
        res.render('usuarios');
      });
    })  
